...
@section('content')
    ...
    @include('back.states.filter.filter')
                    ...
                    <td>
                        <div class="btn-group btn-group-sm">
                            ...
                            <a href="javascript:void(0)"
                               onclick="if(confirm('Are You sure to delete this record?')){document.getElementById('delete-states-{{ $state->id }}').submit();} else {return false}"
                               class="btn btn-danger">
                                <i class="fa fa-trash"></i>
                            </a>
                        </div>
                        <form action="{{ route('admin.states.destroy' , $state->id) }}" method="post" class="d-none" id="delete-states-{{ $state->id }}" >
                            @csrf
                            @method('DELETE')
                        </form>
                    </td>
